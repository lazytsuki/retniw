...
source_inputs: REQUIREMENT-ANALYSIS.md, PRD.md, WORKFLOW-STATE.md, package.json, supabase/config.toml, supabase/migrations/20260824090000_user_review_preferences.sql, supabase/migrations/20260824150000_entry_review_claim.sql, Next.js 16 after 文档, 当前代码, 用户确认 2026-08-24 与 2026-08-26 本地验收反馈
...
| [`ThoughtComposer`](src/components/thoughts/thought-composer.tsx#ThoughtComposer) | 增量 | 初始记录与继续写复用同一组件，通过`data-mode`切换文案和可编辑表面强度；移动端不再隐藏边界 |
...
| `app/review`、`ReviewWorkspace` | 新增并扩展 | 独立承接主动串联、开启说明、联系候选和已经保留的联系，不提供聊天输入 |
| `POST /api/review/scan`、`ReviewService.scanExistingThoughts` | 新增 | 用户主动触发最多20条既有想法的有界扫描，候选继续复用唯一关系真相源 |
...
- 增量修改：移除当前想法末尾的候选和侧栏手动找联系入口。当前想法正文旁只提供一个跳往`/review`的“串联已有想法”链接，不把跨想法能力伪装成当前想法AI。`/review`主区先提供主动串联入口，再按“等你判断 / 已保留”两层展示；每张候选按锚点时间并列显示最多1000字的“后来写的 / 更早写的”原文摘录、简短依据和“保留 / 忽略”。两端均链接`/thoughts/:id#entry-:entryId`，正文entry增加稳定DOM锚点。没有候选时只显示事实空态，不生成总结或推荐话术。
...
- 增量修改：首次进入关闭状态时显示处理范围和“开启并开始串联”按钮；点击后先持久化偏好，只有偏好成功开启才调用主动扫描。已开启时提供“开始串联”和“关闭回看”，偏好更新失败恢复原状态。登录页同步说明：主动使用当前想法AI，或开启回看后，必要内容会交给DeepSeek处理。
...
- 改造前逻辑与历史代码证据：两个entry POST路由等待写入、touch和摘要后响应；旧`POST /api/thoughts/:id/relations/check`读取当前详情，再对最近20个active thought逐个`getDetail`，最后调用[`findConnection`](src/server/ai/deepseek-text-provider.ts#DeepSeekTextProvider)。该路由现已移除，由`ReviewService`统一承接候选生成。
...
- `POST /api/review/scan`无请求体；偏好关闭返回`disabled`，少于2条可用想法返回`not-enough-content`，供应商失败返回可重试的`provider-failed`，候选持久化失败返回可重试的`persistence-failed`及已成功保存数，全部处理完成后返回`processed`及本次实际新增候选数。[设计决策]
