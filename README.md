...
- 想告一段落时可以“先到这里”，以后从以前的想法回来接着写。
- 以前的想法可以移入一个单层合集、归档或删除；删除后无法恢复。
- AI默认不回应。需要时可点“帮我接着想”；整理收进更多操作，跨想法联系集中在“回看”。
...
- 当前想法的AI只在用户主动选择时运行；跨想法回看默认关闭，明确开启后才在保存完成后寻找候选联系。两者都只读取用户写下或导入的内容，不把AI产出当成用户想法。

用户的写作和思考是主体。AI只负责用户明确需要的继续、整理和联系候选；原文不被改写，联系也由用户决定是否保留。
...
内测只使用 [https://retniw.cn](https://retniw.cn)。旧地址 [https://retniw.vercel.app](https://retniw.vercel.app) 的 Vercel 边缘仍以 HTTP 308 跳转到新域名，但当前大陆 DNS 解析可能被污染，不再作为可用入口。线上版本不依赖本机运行，推送当前 GitHub 默认分支后会自动部署。
...
当前不增加邀请后台、邀请码或邮件链接登录。只有主动使用当前想法AI，或明确开启跨想法回看后，必要内容才会交给DeepSeek处理。
