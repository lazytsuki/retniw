...
内测使用稳定的 Vercel 网址，每位体验者拥有独立账号，不共享内容和登录凭据。

1. 在 Vercel 部署生产版本并配置上述五个环境变量。
2. 保持 Supabase 公开注册关闭；在 `Authentication > Users` 中为每位体验者创建一个已确认账号和独立初始密码。
3. 通过一对一渠道分别发送网址、邮箱和初始密码。体验者首次登录后，浏览器会保留会话；手机可按需添加到桌面。
4. 体验者直接记录内容，只有主动点击推进、追问、整理或寻找联系时才会调用 DeepSeek。
5. 内测内容不得包含工作机密或其他不应交给 Supabase、Vercel、DeepSeek 处理的信息。
