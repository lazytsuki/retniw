...
- 边界与不变约束：失败、空值与兼容行为如下。
  - [ ] 保持浮层内部点击不触发外部关闭；打开模态时先关闭非模态层。
  - [ ] 保持键盘用户可到达触发器、菜单项和关闭按钮，关闭后焦点不得丢到页面顶部。
  - [ ] 不得增加玻璃横条或纯装饰容器；保持现有低对比设计语言。
- 前置依赖：无
- 完成定义：
  - [ ] 任意浮层交叉打开时页面只存在一个活动层，四种关闭方式均生效，键盘焦点恢复正确。
- 验证方式：
  - [ ] 入口：运行`npm test -- tests/ui/thought-workspace.test.ts`并用真实浏览器交叉操作账号、更多、历史和导入；被测：[OverlayProvider](src/components/overlay-provider.tsx#OverlayProvider)、[AppHeader](src/components/app-header.tsx#AppHeader)主流程；Mock：自动测试Mock DOM事件，浏览器验收不Mock；断言：单开、外部点击关闭、Escape关闭、完成选择关闭及焦点恢复。
