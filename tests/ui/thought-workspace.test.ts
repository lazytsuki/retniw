...
describe('thought workspace acceptance boundaries', () => {
  ...
  it('has both desktop and mobile workspace layouts', async () => {
    ...
    expect(css).toMatch(/@media \(max-width: 900px\)[\s\S]*\.thought-entry\[id\] \{[\s\S]*scroll-margin-top: 82px/)
  ...
  it('separates history, swipe state, archive, and permanent deletion', async () => {
    ...
    expect(navigation).not.toContain('已删除')
    expect(navigation).toContain('删除后无法恢复，相关联系也会一并删除。')
    expect(navigation).toContain("fetch(`/api/thoughts/${thought.id}`, { method: 'DELETE' })")
    expect(navigation).toContain("response.status !== 204")
